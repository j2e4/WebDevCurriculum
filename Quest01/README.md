...
* HTML 문서는 어떤 구조로 이루어져 있나요?
  ...
  * `<head>`에 자주 들어가는 엘리먼트들은 어떤 것이 있고, 어떤 역할을 할까요?
    ...
    문서를 표현하기 위해 먼저 처리해야하는 메타 데이터를 담는다.
    - `<title>`: 브라우저의 제목 표시줄, 탭에 표시되는 문서의 제목
    - `<base>`: 문서 안에서 사용하는 상대 경로의 기준 URL
    - `<link>`: 외부 리소스를 연결하는 요소 ex) stylesheet, favicon
    - `<style>`: 문서에 적용할 스타일 정보
    - `<meta>`: 다른 메타 요소로 나타낼 수 없는 메타 정보 ex) charset, content, http-equiv
    - `<script>`: 스크립트를 포함하거나 외부 스크립트를 연결하는 요소
    - `<noscript>`: 스크립트가 적용될 수 없을 때 보여줄 HTML 구획
    - `<template>`: 스크립트에 의해 채워질 HTML 코드를 담는 요소
  ...
  * 시맨틱 태그는 무엇일까요?
    ...
    * 시맨틱 엘리먼트를 사용하면 어떤 점이 좋을까요?
      ...
      의미가 명확한 태그이기 때문에 다음과 같은 이점이 있다.
      - 유지보수를 할 때 내용을 파악하는 데 걸리는 시간을 줄일 수 있다.
      - 검색 엔진이 페이지를 분석할 때 SEO(검색 엔진 최적화)에 도움이 된다.

    * `<section>`과 `<div>`, `<header>`, `<footer>`, `<article>` 엘리먼트의 차이점은 무엇인가요?
      - `<section>`: 서로 연관이 있는 문단의 내용을 구분하는 태그
      - `<div>`: non-semantic 태그로 그저 block하게 내용을 묶는 태그
      - `<header>`: 앞으로 주어지는 내용을 대표 또는 소개하는 태그
      - `<footer>`: 지금까지 주어진 내용을 요약하거나 관련 내용을 소개하거나 작가, 저작권, 연락 정보를 기재하는 등 내용을 마무리하는 태그
      - `<article>`: 다른 내용들과 연관이 없고 독립적으로 스스로의 내용을 가지는 태그

  * 블록 레벨 엘리먼트와 인라인 엘리먼트는 어떤 차이가 있을까요?

    블록 레벨의 요소(`<div>`)는 주어진 너비 전체를 사용하는 요소이고 인라인 요소(`<span>`)는 내용을 표현하는데 필요한 만큼의 너비를 사용한다.
## Quest
